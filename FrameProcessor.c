...
ssize_t xfifo_dma_read(struct file *filp, char __user
...
ssize_t xfifo_dma_write(struct file *filp, const char __user
...
dev->
buffer_v_addr = dma_alloc_coherent( && dev->pdev->dev, transfer_size,
...
set_dma_mode(dev
->dma_channel, DMA_MODE_WRITE);
set_dma_addr(dev
->dma_channel, dev->buffer_d_addr);
set_dma_count(dev
->dma_channel, transfer_size);
set_pl330_client_data(dev
->dma_channel, dev->client_data);
set_pl330_done_callback(dev
->dma_channel,
xfifo_dma_done_callback, dev);
set_pl330_fault_callback(dev
->dma_channel,
xfifo_dma_fault_callback, dev);
set_pl330_incr_dev_addr(dev
->dma_channel, 0);

/* Load our DMA buffer with the user data */
copy_from_user(dev
->buffer_v_addr, buf, transfer_size);
...
wait_event_interruptible(xfifo_dma_wait, dev
->busy == 0);
